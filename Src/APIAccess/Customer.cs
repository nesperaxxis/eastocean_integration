...
{
    internal class Customer : WMSSynchObject
    {
        internal const string _wmsAPIEndPoint = "Customer";        //TODO: Provide the Simplr WMS API EndPoint to call here, excluding the BaseAddress
        internal const string _objectType = SBOAddon.SYNCH_O_OBJECT_CUSTOMER;
        ...
        //public List<CustomerDetails> ShipToAddress = new List<CustomerDetails>();
        ...
        public String Active = "";     //Required    Active(Y/N) // changed to 1/0 as required by Simplr
        ...
        public Customer(string code) : base()
        {
            _CardCode = code;
            ...
            //Change to required value by Simplr 1 or 0
            ...
            FaxNumber = String.IsNullOrEmpty(rs.Fields.Item("Fax").Value.ToString()) ? "" : rs.Fields.Item("Fax").Value.ToString();
            Email = String.IsNullOrEmpty(rs.Fields.Item("E_Mail").Value.ToString()) ? "" : rs.Fields.Item("E_Mail").Value.ToString();
            WebSite = String.IsNullOrEmpty(rs.Fields.Item("IntrntSite").Value.ToString()) ? "" : rs.Fields.Item("IntrntSite").Value.ToString();
    ...
    internal class ShipToAddress
    {
        ...
        public static List<ShipToAddress> GetShipToAddress(string cardCode)
        {
            List<ShipToAddress> addresses = new List<ShipToAddress>();
            string sql = String.Format(Resource.Queries.GET_RECORD_CRD1_SHIPTO, cardCode.Replace("'", "''"));
            var rs = eCommon.ExecuteQuery(sql);
            for (int i = 0; i < rs.RecordCount; i++)
            {
                addresses.Add(new ShipToAddress
                    {
                        CardCode = rs.Fields.Item("CardCode").Value.ToString(),
                        RowNumber = (int)rs.Fields.Item("LineNum").Value,
                        AddressName = String.IsNullOrEmpty(rs.Fields.Item("Address").Value.ToString()) ? "" : rs.Fields.Item("Address").Value.ToString(),
                        Street = String.IsNullOrEmpty(rs.Fields.Item("Street").Value.ToString()) ? "" : rs.Fields.Item("Street").Value.ToString(),
                        Block = String.IsNullOrEmpty(rs.Fields.Item("Block").Value.ToString()) ? "" : rs.Fields.Item("Block").Value.ToString(),
                        PostCode = String.IsNullOrEmpty(rs.Fields.Item("ZipCode").Value.ToString()) ? "" : rs.Fields.Item("ZipCode").Value.ToString(),
                        City = String.IsNullOrEmpty(rs.Fields.Item("City").Value.ToString()) ? "" : rs.Fields.Item("City").Value.ToString()
                    }
                );
                rs.MoveNext();
            }
